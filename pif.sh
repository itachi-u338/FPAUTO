...
grep -o 'href=".*download.*"' PIXEL_LATEST_HTML | grep -q 'qpr' || NOT_QPR='-v';
wget -q -T 10 -O PIXEL_FI_HTML --no-check-certificate "https://developer.android.com$(grep -o 'href=".*download.*"' PIXEL_LATEST_HTML | grep -v 'ota' | grep $NOT_QPR qpr | cut -d\" -f2 | sort -ru | head -n1)" 2>&1 || exit 1;
wget -q -T 10 -O PIXEL_OTA_HTML --no-check-certificate "https://developer.android.com$(grep -o 'href=".*download-ota.*"' PIXEL_LATEST_HTML | grep $NOT_QPR qpr | cut -d\" -f2 | sort -ru | head -n1)" 2>&1 || exit 1;
